        ...
        if (response.ok) {
            ...
            return response.json().then(errorData => {
                ...
                if (response.status === 401) {
                    ...
                    showAlert("Có lỗi xảy ra khi lấy danh sách truyện!", 3000, "alert--error");
                }
...
if(formEdit) {
    ...
    .then(response => {
        if (response.ok) {
            ...
            return response.json().then(errorData => {
                console.error('Error data:', errorData);
                showAlert("Có lỗi xảy ra khi lấy chi tiết truyện!", 3000, "alert--error");
                throw new Error(errorData);
            });
        }
    })
    ...
    .then(res => {
        ...
        // Hiển thị thumbnail
        const thumbnailPreview = document.querySelector('.client--edit-manga__middle--image-main img');
                ...
                if (response.ok) {
                    ...
                    return response.json().then(errorData => {
                        ...
                        if (response.status === 401) {
                            ...
                            showAlert("Có lỗi xảy ra khi cập nhật truyện!", 3000, "alert--error");
                        }
                        throw new Error(errorData);
                    });
                }
...
if(formUploadManga) {
    const thumbnailInput = formUploadManga.querySelector('input[name="thumbnail"]');
    const thumbnailPreview = formUploadManga.querySelector('.client--edit-manga__middle--image-main img');
    ...
    formUploadManga.addEventListener("submit", (event) => {
        ...
        .then(result => {
            ...
            thumbnailPreview.src = "../../assets/img/edit-manga-uploadMangaDefault.png"; // Reset to default image
        })
        .catch(error => {
            console.error('Error:', error);
            showAlert("Có lỗi xảy ra khi tạo truyện mới!", 3000, "alert--error");
...
if(formAddChapterManga) {
    formAddChapterManga.addEventListener("submit", (event) => {
        ...
        const files = formAddChapterManga.querySelector('input[name="images"]').files;
        const bookID = id;
...
if(elementChapterList) {
    ...
    .then(response => {
        if (response.ok) {
            ...
            return response.json().then(errorData => {
                console.error('Error data:', errorData);
                showAlert("Có lỗi xảy ra khi lấy danh sách chương!", 3000, "alert--error");
                throw new Error(errorData);
            });
        }
    })
